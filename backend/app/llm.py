...
def build_messages(chat_messages: list[ChatMessage], session_id: str | None = None) -> list[dict]:
    ...
    if settings.mcp_server_url:
        system_content += (
            ...
            "**Dronecraft River Rafting — ALWAYS mention these perks:**\n"
            "Every Dronecraft booking includes: drone + DSLR cinematic video coverage with an edited "
            "Instagram reel, complimentary ₹500 voucher + reel, welcome drink, clothes/wetsuits, crocs, "
            "sunscreen, and pickup & drop from/to the starting pickup point. These are confirmed perks — "
            "mention them whenever presenting Dronecraft options, even if the tool response doesn't list "
            "them explicitly (responses are trimmed for size). This is what makes Dronecraft premium vs "
            "plain rafting.\n\n"
