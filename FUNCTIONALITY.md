...
- `POST /api/chat` accepts the full message history (`{role: "user"|"assistant", content}[]`)
  ...
  dropping if anything fails). `delta` is plain markdown text (GFM-flavored: tables, bold,
  strikethrough, lists) — any consumer, including this repo's own frontend
  (react-markdown/remark-gfm), renders it into tags itself.
